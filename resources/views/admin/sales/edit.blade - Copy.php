...
@section('content')
            ...
            <form action="{{ route('purchases.update', ['purchase' => $purchase->id]) }}" method="POST" id="purchaseform">
                    ...
                    <div class="form-group col-md-4  mt-2">
                        ...
                        <div>
                            <input type="date" name="real_date" id="real_date" class="form-control"
                                value="{{ $purchase->real_date }}" required>
                        </div>
                    </div>
                    <div class="form-group col-md-4  mt-2">
                            ...
                            <select name="supplier_id" id="supplier_id" class="form-select" required>
                                ...
                                @foreach ($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}"
                                        {{ $supplier->id == $purchase->supplier_id ? 'selected="true"' : '' }}>
                            ...
                            <select name="warehouse_id" id="warehouse_id" class="form-select" required>
                                ...
                                @foreach ($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}"
                                        {{ $warehouse->id == $purchase->warehouse_id ? 'selected="true"' : '' }}>
                ...
                <div class="table-responsive">
                    <table class="table table-hover" id="purchase-details">
                        ...
                        <tbody>
                            @foreach ($purchaseDetails as $k => $detail)
                                <tr id="{{ $detail->product_id }}" subtotal="{{ $detail->total }}">
                                    ...
                                    <td>
                                        <div role="group" class="input-group">
                                            <div class="input-group-prepend"><span class="btn btn-primary btn-sm minus"
                                                    onclick="decrementValue({{ $detail->product_id }})">-</span></div>
                                    ...
                                    <td id="discount-{{ $detail->product_id }}" discount="{{ $detail->discount }}"
                                        ...
                                        original_cost_price="{{ $detail->price }}" original_tax="{{ $detail->tax }}"
                                    ...
                                    <td><a href="javascript:void(0)" class="btn btn-primary "
                                            onclick="editpurchase({{ $detail->product_id }})"><i
                                                class="bi bi-pencil-square"></i></a><a href="javascript:void(0)"
                                            class="btn btn-danger ml-5"
                                            onclick="deletePurchase({{ $detail->product_id }})"><i
                        ...
                        <table class="table table-striped table-sm">
                            <tbody>
                                <tr>
                                    ...
                                    <td><span id="tax-whole-purchase"> 0.00 ({{ $purchase->order_tax }} %)</span></td>
                                </tr>
                                <tr>
                                    <td class="bold">{{ __('Discount') }}</td>
                                    <td id="discount-whole-purchase">{{ $purchase->discount }}</td>
                                </tr>
                                <tr>
                                    <td class="bold">{{ __('Shipping') }}</td>
                                    <td id="shipment-whole-purchase">{{ $purchase->shippment_price }}</td>
                                </tr>
                                <tr>
                                    <td><span class="font-weight-bold">{{ __('Grand Total') }}</span>
                                    </td>
                                    <td><span class="font-weight-bold"
                                            id="grand-total">{{ $purchase->grand_total }}</span></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="form-group col-md-3 mt-2">
                        ...
                        <div>
                            <input type="number" name="discount" id="order_discount" class="form-control"
                                value="{{ old('discount', $purchase->discount) }}" min="0"
                    ...
                    <div class="form-group col-md-3 mt-2">
                        ...
                        <div>
                            <input type="number" name="shippment_price" id="shippment_price" class="form-control"
                                value="{{ old('shippment_price', $purchase->shippment_price) }}" min="0"
                            ...
                            <select name="status" class="form-select" id="order_status">
                                @foreach ($purchaseStatuses as $status)
                                    <option value="{{ $status->id }}"
                                        {{ $status->id == $purchase->status ? 'selected="true"' : '' }}>
                ...
                <div class="form-group row mt-2 d-none">
                    ...
                    <div class="col-md-6">
                        <input type="hidden" name="grand_total" id="grand_total_tosend" class="form-control"
                            value="{{ old('grand_total', $purchase->grand_total) }}" placeholder="Grand Total"
                            step="any" required>
                    </div>
                </div>
                ...
                <div class="form-group row mt-3">
                    ...
                    <div>
                        <textarea name="comment" id="comment" placeholder="{{ __('Comment') }}" class="form-control">{{ old('comment', $purchase->comment) }}</textarea>
                    </div>
                </div>
    ...
    <script>
        ...
        function savealldata(event) {
            ...
            fetch('/admin/purchases/{{ $purchase->id }}?' + added_data, {
                    method: 'PUT',
                    body: JSON.stringify({
                        data
                    })
