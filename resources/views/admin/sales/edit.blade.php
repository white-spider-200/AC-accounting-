...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('sales.update', ['sale' => $sale->id]) }}" method="POST" id="saleform">
                @csrf
                @method('PUT')
                <div class="row mb-5">
                            ...
                            <select name="client_id" id="client_id" class="form-select" required>
                                ...
                                @foreach ($clients as $client)
                                    <option value="{{ $client-> id }}"
                                        {{ $client-> id == $sale-> client_id ? 'selected="true"' : '' }}>
                                        {{ $client-> name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group col-md-4  mt-2">
                        ...
                        <div>
                            <select name="warehouse_id" id="warehouse_id" class="form-select" required disabled>
                                <option value=""> ... </option>
                                @foreach ($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}"
                                        {{ $warehouse->id == $sale->warehouse_id ? 'selected="true"' : '' }}>
                    ...
                    <table class="table table-hover" id="table-of-details">
                        ...
                        <tbody>
                            @foreach ($saleDetails as $k => $detail)
                                <tr id="{{ $detail->product_id }}" subtotal="{{ $detail->total }}">
                                    <td>{{ $k + 1 }}</td>
                                    <td>{{ $detail->product->code }}<br><span
                                            class="badge bg-success">{{ app()->getLocale() == 'ar' ? $detail->product->label_ar : $detail->product->label_en }}</span>
                                    </td>
                                    <td id="cost_price-{{ $detail->product_id }}">{{ $detail->price }}</td>
                                    <td>{{ $detail->in_warehouse }}</td>
                                    <td>
                                        <div role="group" class="input-group">

                                            <div class="input-group-prepend"><span class="btn btn-primary btn-sm minus"
                                                    onclick="decrementValue({{ $detail->product_id }})">-</span>
                                                </div>
                                            <input min="0" value="{{ $detail->qty }}" class="form-control qtyfield"
                                                type="text" id="qty-{{ $detail->product_id }}">
                                            <div class="input-group-append"><span class="btn btn-primary btn-sm plus"
                                                    onclick="incrementValue({{ $detail->product_id }})">+</span></div>
                                                    <div id="msg-q-{{ $detail->product_id }}"></div>
                                        </div>
                                    </td>
                                    <td id="discount-{{ $detail->product_id }}" discount="{{ $detail->discount }}"
                                        discount-type="{{ $detail->discount_type }}" cost_price="{{ $detail->price }}"
                                        original_cost_price="{{ $detail->price }}" current_stock="{{@$detail->getProductStockQuantity($sale->warehouse_id, $detail->product_id)}}" original_tax="{{ $detail->tax }}"
                                        currency="{{ $detail->currency }}">{{ $detail->discount }}
                                        {{ $detail->currency }}</td>
                                    <td tax="{{ $detail->tax_value_beforeqty }}" id="tax-{{ $detail->product_id }}">
                                        {{ $detail->tax_price }} {{ $detail->currency }}</td>
                                    <td id="subtotal-{{ $detail->product_id }}">{{ $detail->total }}
                                        {{ $detail->currency }}</td>
                                    <td><a href="javascript:void(0)" class="btn btn-primary "
                                            onclick="editsale({{ $detail->product_id }})"><i
                                                class="bi bi-pencil-square"></i></a><a href="javascript:void(0)"
                                            class="btn btn-danger ml-5"
                                            onclick="deletesale({{ $detail->product_id }})"><i
                        ...
                        <table class="table table-striped table-sm">
                            <tbody>
                                <tr>
                                    ...
                                    <td><span id="tax-whole-sale"> 0.00 ({{ $sale->order_tax }} %)</span></td>
                                </tr>
                                <tr>
                                    <td class="bold">{{ __('Discount') }}</td>
                                    <td id="discount-whole-sale">{{ $sale->discount }}</td>
                                </tr>
                                <tr>
                                    <td class="bold">{{ __('Shipping') }}</td>
                                    <td id="shipment-whole-sale">{{ $sale->shippment_price }}</td>
                                </tr>
                                <tr>
                                    <td><span class="font-weight-bold">{{ __('Grand Total') }}</span>
                                    </td>
                                    <td><span class="font-weight-bold"
                                            id="grand-total">{{ $sale->grand_total }}</span></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    @php
                        $currentOrderTax = (string) old('order_tax', $sale->order_tax);
                    ...
                    <div class="form-group col-md-3 mt-2">
                        ...
                        <div>
                            ...
                            <input type="hidden" min="0" name="order_tax" id="order_tax" class="form-control"
                                value="{{ old('order_tax', $sale->order_tax) }}" step="any" required>
                        </div>
                    </div>
                    <div class="form-group col-md-3 mt-2">
                    ...
                    <div class="form-group col-md-3 mt-2">
                        ...
                        <div>
                            <input type="number" name="shippment_price" id="shippment_price" class="form-control"
                                value="{{ old('shippment_price', $sale->shippment_price) }}" min="0"
                                placeholder="Shipment Price" step="any" required>
                        </div>
                    </div>
                    <div class="form-group col-md-3 mt-2">
                            ...
                            <select name="status" class="form-select" id="order_status">
                                @foreach ($salesStatuses as $status)
                                    <option value="{{ $status->id }}"
                                        {{ $status->id == $sale->status ? 'selected="true"' : '' }}>
                ...
                <div class="form-group row mt-2 d-none">
                    ...
                    <div class="col-md-6">
                        <input type="hidden" name="grand_total" id="grand_total_tosend" class="form-control"
                            value="{{ old('grand_total', $sale->grand_total) }}" placeholder="Grand Total"
                            step="any" required>
                    </div>
                </div>
                ...
                <div class="form-group row mt-3">
                    ...
                    <div>
                        <textarea name="comment" id="comment" placeholder="{{ __('Comment') }}" class="form-control">{{ old('comment', $sale->comment) }}</textarea>
                    </div>
                </div>
    ...
    <script>
        ...
        function incrementValue(id) {
            ...
            if( quantity > currentStock || quantity ==  currentStock){
                document.getElementById('msg-q-'+id).innerHTML = '<b class="red msg-qty">'+'{{__("The Heighst No. is")}} '+currentStock+'</b>';
                return false;
            }else{
        ...
        function getgrandtotal() {
            ...
            if (taxValue > -1 && taxValue != '') {
                ...
                if (!isNaN(taxAfter)) {
                    document.getElementById('tax-whole-sale').textContent = taxAfter + " " + '(' + taxValue + '%)';
                }
            ...
            if (parseFloat(discount) > parseFloat(sum)) {
                ...
                document.getElementById('discount-whole-sale').innerHTML =
            ...
            if (discount > -1 && (parseFloat(discount) < parseFloat(sum))) {
                ...
                document.getElementById('discount-whole-sale').textContent = discount;
                ...
                if (!isNaN(taxAfter)) {
                    document.getElementById('tax-whole-sale').innerHTML = taxAfter + " " + '(' + taxValue + '%)';
                }
            ...
            if (shippmentPrice.value > -1 && shippmentPrice.value != '') {
                ...
                document.getElementById('shipment-whole-sale').textContent = shippmentPrice.value;
        ...
        function savealldata(event) {
            ...
            //order_tax order_discount shippment_price  comment grand-total tax-whole-sale client_id
            ...
            fetch('/admin/sales/{{ $sale->id }}?' + added_data, {
                    method: 'PUT',
                    body: JSON.stringify({
                        data
                    })
