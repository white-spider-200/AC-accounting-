...
@section('content')
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">{{ __('Edit Adjustment') }}</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('adjustments.update', ['adjustment' => $adjustment-> id]) }}" method="POST" id="adjustmentform">
                @csrf
                @method('PUT')
                <div class="row mb-5">
                    <div class="form-group col-md-4  mt-2">
                        ...
                        <div>
                            ...
                            value="{{ $adjustment-> real_date }}" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group col-md-4  mt-2">
                        ...
                        <div>
                            <select name="warehouse_id" id="warehouse_id" class="form-select" required @readonly(true)>
                                <option value=""> ... </option>
                                @foreach ($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}" {{ ($warehouse-> id == $adjustment-> warehouse_id ) ? 'selected="true"':'' }}> {{ $warehouse->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <table class="table table-hover" id="purchase-details">
                        ...
                        <tbody>
                            ...
                            <tr id="{{ $detail-> product_id }}">
                                ...
                                <td>
                                    <div role="group" class="input-group">
                                        <div class="input-group-prepend"><span class="btn btn-primary btn-sm minus"
                                                onclick="decrementValue({{ $detail-> product_id }})">-</span></div><input min="0" value="{{ $detail-> qty }}"
                                            class="form-control qtyfield" type="text" id="qty-{{ $detail-> product_id }}">
                                ...
                                <td>
                                    <select id="adjustment_type-{{ $detail-> product_id }}">
                                        ...
                                        <a href="javascript:void(0)" class="btn btn-danger ml-5" onclick="deleteAdjustment('{{ $detail-> product_id }}')"><i class="bi bi-trash"></i></a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
    ...
    <script>
        ...
        function savealldata(event) {
            ...
            fetch('/admin/adjustments/{{ $adjustment-> id }}?' + added_data, {
                    method: 'PUT',
                    body: JSON.stringify({
                        data
                    })
