...
<div class="card">
    <div class="card-body py-4">
        <form method="GET" action="{{ route('accounting.profit-loss') }}" class="row g-3 align-items-end">
            <div class="col-md-2">
        ...
        <div class="d-flex gap-2 flex-wrap my-3">
            <a href="{{ route('accounting.profit-loss', array_merge(request()->query(), ['export' => 'excel'])) }}" class="btn btn-outline-success btn-sm">{{ __('Export Excel') }}</a>
            <a href="{{ route('accounting.profit-loss', array_merge(request()->query(), ['export' => 'pdf'])) }}" class="btn btn-outline-danger btn-sm" target="_blank">{{ __('Export PDF') }}</a>
        </div>

        <table class="table table-bordered mb-0">
            <tbody>
                @foreach ($summary as $row)
                    <tr @if (!empty($row['highlight'])) class="table-success" @endif>
                        <th>{{ __($row['label']) }}</th>
