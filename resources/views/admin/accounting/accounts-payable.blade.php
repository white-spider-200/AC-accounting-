...
<div class="card">
    <div class="card-body py-4">
        <form method="GET" action="{{ route('accounting.accounts-payable') }}" class="row g-3 align-items-end">
            <div class="col-md-2">
        ...
        <div class="d-flex gap-2 flex-wrap my-3">
            <a href="{{ route('accounting.accounts-payable', array_merge(request()->query(), ['export' => 'excel'])) }}" class="btn btn-outline-success btn-sm">{{ __('Export Excel') }}</a>
            <a href="{{ route('accounting.accounts-payable', array_merge(request()->query(), ['export' => 'pdf'])) }}" class="btn btn-outline-danger btn-sm" target="_blank">{{ __('Export PDF') }}</a>
        </div>

        <div class="row g-3 mb-3">
            <div class="col-md-4">
                <div class="alert alert-info mb-0"><strong>{{ __('Total Payables') }}:</strong> {{ number_format($totalPayables, 2) }}</div>
            </div>
            <div class="col-md-8">
